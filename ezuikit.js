...
(function (global, factory) {
  ...
  var EZUIPlayer = function (playParams) {
    ...
    if (
      ...
      playParams.hasOwnProperty("decoderPath")
    ) {
      ...
      this.loadingStart = function () {
        ...
        if (playParams && playParams.id) {
          ...
          var windowHeight =
            domElement.offsetHeight || playParams.height || 400;
          var offsetTop = domElement.offsetTop;
          var offsetLeft = domElement.offsetLeft;
          // 先执行清空loading
          if (document.getElementById("loading-id-0")) {
            document
              .getElementById("loading-id-0")
              .parentNode.removeChild(document.getElementById("loading-id-0"));
          }
          ...
          for (var i = 0; i < windowLength; i++) {
            ...
            var style =
              ...
              windowWidth / splitBasis +
              "px;height:" +
              windowHeight / splitBasis +
              "px;outline:none;vertical-align: top;position:absolute;";
            style +=
              "left:" +
              calLoadingPostion(windowHeight, windowWidth, splitBasis, i).left +
              "px;";
            style +=
              "top:" +
              calLoadingPostion(windowHeight, windowWidth, splitBasis, i).top +
              "px;";
            loadingContainer.setAttribute("style", style);
            function calLoadingPostion(
              windowHeight,
              windowWidth,
              splitBasis,
              i
              ...
              var top =
                parseInt(i / splitBasis, 10) * (windowHeight / splitBasis);
      ...
      this.loadingSet = function (index, opt) {
        ...
        if (document.getElementById("loading-item-" + index)) {
          var textElement = document.getElementById("loading-item-" + index)
            .childNodes[1];
      ...
      this.loadingEnd = function (index) {
        var loadingItemContainerDOM = document.getElementById(
          "loading-item-" + index
        );
        if (loadingItemContainerDOM) {
          loadingItemContainerDOM.parentNode.removeChild(
            loadingItemContainerDOM
          );
          var loadingContainerDOM = document.getElementById("loading-id-0");
          if (
            loadingContainerDOM &&
            loadingContainerDOM.children.length === 0
          ) {
      ...
      if (width) {
        ...
        this.log(
          "video width:" + this.opt.width + " height:" + this.opt.height
        );
      } else {
  ...
  EZUIPlayer.prototype.getRealUrl = function (playParams) {
    ...
    if (playParams && playParams.hasOwnProperty("decoderPath")) {
      if (playParams.url.indexOf("open.ezviz.com") !== -1) {
        ///海外
        ...
        if (!/^ezopen:\/\//.test(ezopenURL)) {
          ...
          var apiSuccess = function (data) {
            if (data.code == 200 || data.retcode == 0) {
              ...
              /**参数容错处理  start*/
              if (playUrl.indexOf("playback") !== -1) {
                //回放
                var wsBegin =
                  getQueryString("begin", playUrl) ||
                  getQueryString("begin", playParams.url);
                var wsEnd =
                  getQueryString("end", playUrl) ||
                  getQueryString("end", playParams.url);
                // 兼容各种时间格式
                if (!wsBegin) {
                  ...
                  realUrl = realUrl.replace(
                    "&begin=" + getQueryString("begin", playUrl),
                    "&begin=" + formatRecTime(wsBegin, "000000")
                  );
                ...
                if (!wsEnd) {
                  ...
                  realUrl = realUrl.replace(
                    "&end=" + getQueryString("end", playUrl),
                ...
                if (playParams.url.indexOf(".cloud") !== -1) {
                  ...
                  var recBegin = reRormatRecTime(
                    getQueryString("begin", realUrl)
                  );
                  ...
                  function recAPISuccess(data) {
                    if (data.code == 200) {
                      ...
                      _this.loadingSet(0, {
                        text:
                          _this.opt.domain !== "open"
                            ? "Get device live view address complete"
                            : "获取设备播放地址",
                      });
                      resolve(
                        JSON.stringify({
                    ...
                    function recSliceArrFun(data) {
                      ...
                      data.forEach(function (item, index) {
                        if (
                          ...
                          downloadPathArr[downloadPathArr.length - 1].endTime =
                            item.endTime;
                        }
                      });
                  ...
                  if (playParams.url.indexOf("alarmId") !== -1) {
                    ...
                    var recEnd = reRormatRecTime(
                      getQueryString("end", realUrl)
                    );
                    var deviceSerial = getQueryString("serial", realUrl);
                    var channelNo = getQueryString("chn", realUrl);
                    ...
                    function recAPISuccess(data) {
                      if (data.code == 200) {
                        ...
                        if (data.data) {
                          ...
                          realUrl +=
                            "&recSlice=" + recSliceArrJSON.replace("\\", "");
      ...
      urlList.map(function (item, index) {
        ...
        var adaptItem = item.replace(
          adaptHost,
          _this.opt.domain !== "open" ? "open.ezviz.com" : "open.ys7.com"
        );
            ...
            if (playParams.websocketParams) {
              Object.keys(playParams.websocketParams).map(function (item) {
                websocketStreamingParam +=
                  "&" + item + "=" + playParams.websocketParams[item];
              });
            }
  ...
  EZUIPlayer.prototype.initDecoder = function (playParams) {
    ...
    this.log("初始化解码器---开始");
    var _this = this;
    _this.opt.domain = playParams.url.match(
      /ezopen:\/\/(\S*)\.(ys7|ezviz)\.com/
    )[1];
    if (playParams.env) {
      var environmentParams = playParams.env;
      domain = environmentParams.domain;
    ...
    function initDecoder(resolve, reject) {
      ...
      addJs(jsPluginPath, function () {
        ...
        _this.jSPlugin = new JSPlugin({
          ...
          iCurrentSplit:
            playParams.splitBasis ||
            Math.ceil(Math.sqrt(playParams.url.split(",").length)),
        ...
        window.onresize = function () {
          _this.jSPlugin.JS_Resize(
            playParams.width || 600,
            playParams.height || 400
          );
        };
        _this.log("初始化解码器----完成");
        if (playParams.controls && playParams.controls.length > 0) {
          var videoControlsJsPath =
            playParams.decoderPath + "/deviceControls/videoControls.js";
          var videoControlsCssPath =
            playParams.decoderPath + "/deviceControls/deviceControls.css";
          addJs(videoControlsJsPath, function () {
            addCss(videoControlsCssPath, function () {
              ...
              document.getElementById(playParams.id).style.position =
                "relative";
              document
                .getElementById(playParams.id)
                .appendChild(VideoControlsDOM);
  ...
  EZUIPlayer.prototype.stop = function (i, unDestory) {
    ...
    if (!!window["CKobject"]) {
      ...
      if (typeof i === "undefined") {
        ...
        return this.jSPlugin.JS_Stop(i).then(
          function () {
  ...
  // 视频截图
  EZUIPlayer.prototype.capturePicture = function (
    iWind,
    pictureName,
    callback,
    isUndownload
    ...
    if (!!this.jSPlugin) {
      return this.jSPlugin.JS_CapturePicture(
        iWind,
        pictureName,
        "JPEG",
        callback,
